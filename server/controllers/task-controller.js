    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({
                error: `User with ID ${userId} not found. Please check the user ID and try again.`
            }
            );
        } else {
            ...
            if (tasks.length === 0) {
                return res.status(404).json({
                    error: `No tasks found for user with ID ${userId}.`
                }
                );
            } else {
        ...
        return res.status(500).json({
            error: `Internal server error: Failed to retrieve tasks for user: ${userId}. Please try again later.`
        });
    }
};
...
const getTaskById = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            if (!task) {
                return res.status(404).json({
                    error: `Task with ID:${taskId} not found for user with ID:${userId}.`
                }
                );
            } else {
        ...
        return res.status(500).json({
            error: `Internal server error: Failed to retrieve task with ID ${taskId} for user with ID ${userId}. Please try again later.`
        }
        );
    }
};
...
const modifyTask = async (req, res) => {
    ...
    try {
        const columns = await knex(`tasks`)
            .columnInfo();
